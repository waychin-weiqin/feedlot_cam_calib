...
\paragraph{Motivation for learned refinement.} This pipeline reduces ATE by 10\% (0.177 $\rightarrow$ 0.161) but leaves ARE essentially unchanged (4.37\degree{} $\rightarrow$ 4.39\degree{}). The residual rotation error is systematic---not reducible by further classical aggregation---because the scene provides weak geometric signal (flat pen, repetitive dynamic content). This motivates exploiting Pi3X's conditioning interface to inject prior information.
...
Applying the same geometric post-processing to $\{T_k^{c,(1)}\}$ yields a refined estimate. We observe:
\begin{itemize}
    \item ARE improves: 4.39\degree{} $\rightarrow$ 3.49\degree{} (20\% reduction)
    \item ATE degrades: 0.161 $\rightarrow$ 0.166 (3\% increase)
\end{itemize}

Iterating na\"ively amplifies both effects---rotation continues to improve (to $\sim$3.3\degree{}) while translation degrades monotonically (to 0.195 after 5 iterations). Section~\ref{sec:theory} provides a theoretical explanation: the rotation component contracts toward a fixed point on SO(3), while translations diverge due to Sim(3) scale mismatch.
