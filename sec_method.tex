...
Our method operates in three stages. Stage~1 applies standard geometric post-processing to establish a baseline estimate. Stages~2--3 constitute our contribution: self-conditioned iterative refinement with decoupled rotation-translation fusion.
...
Pi3X supports multimodal conditioning: given approximate poses $\hat{T}^c$ and camera intrinsic matrices $\boldsymbol{\Pi}^c \in \mathbb{R}^{3 \times 3}$, it refines its predictions via two encode-path mechanisms: (1)~ray embeddings that convert pixel coordinates to 3D directions using intrinsics and poses, added to image features before decoding; and (2)~scale normalisation that uses pose translations to set proper depth scale. We verify empirically that the decode-path PoseInjectBlocks (PRoPE attention injection) contribute nothing when ablated (Section~\ref{sec:ablation_mechanism}). We condition on the Stage~1 output:
\begin{equation}
...
Since a single conditioned pass reduces rotation error by 20\%, a natural question is whether iterating the process yields further gains. We apply hybrid fusion iteratively: at each step, the optimised rotation estimate from the previous iteration serves as the new conditioning input. This implements block coordinate descent on an implicit joint pose-feature objective: each iteration updates features (given poses) via the encode-path ray embeddings, then updates poses (given improved features) via the decoder.
...
\paragraph{Stochastic extension.} One might expect that perturbing conditioning poses and averaging across multiple samples (Monte Carlo integration) could further reduce error. We investigate this extension in Section~\ref{sec:ablation_S} and find it provides no benefit: deterministic iteration ($S=1$) is optimal, consistent with the block coordinate descent interpretation where the mechanism is deterministic feature enrichment rather than stochastic denoising.
